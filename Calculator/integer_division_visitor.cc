...
MathElementPtr IntegerDivisionVisitor::VisitInteger(const Integer* integer) const {
    if (operand_->value() % integer->value() == 0) {
        // if the operand is divisible by the integer, just divide them and return a new integer
...
// Visit an Exponent
MathElementPtr IntegerDivisionVisitor::VisitExponent(const Exponent* exponent) const {
